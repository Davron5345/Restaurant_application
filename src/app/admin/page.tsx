...
export default async function AdminReportsPage() {
  ...
  try {
    ...
    console.error("Prisma error in Reports:", error);
    return (
      <div style={{ padding: "24px", color: "#ef4444" }}>
        <h2>Ошибка загрузки данных</h2>
        <p>{error.message}</p>
        <p>Убедитесь, что база данных подключена и выполнен <code>npx prisma db push</code>.</p>
      </div>
    );
  }
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "800px" }}>
          <thead>
            <tr style={{ background: "#f1f5f9", textAlign: "left" }}>
              ...
              <th style={{ padding: "12px", borderBottom: "1px solid #e2e8f0", color: "#64748b", fontWeight: "600" }}>Филиал</th>
              ...
              <th style={{ padding: "12px", borderBottom: "1px solid #e2e8f0", color: "#64748b", fontWeight: "600" }}>Комментарий</th>
            </tr>
          </thead>
          <tbody>
            {transactions.map(t => (
              <tr key={t.id} style={{ transition: "background 0.2s" }} onMouseOver={e => e.currentTarget.style.background = "#f8fafc"} onMouseOut={e => e.currentTarget.style.background = "transparent"}>
                ...
                <td style={{ padding: "12px", borderBottom: "1px solid #e2e8f0" }}>
                  {t.branch?.name || <span style={{ color: "#94a3b8" }}>Общий</span>}
                </td>
                <td style={{ padding: "12px", borderBottom: "1px solid #e2e8f0" }}>
                ...
                <td style={{ padding: "12px", borderBottom: "1px solid #e2e8f0", fontSize: "14px", color: "#64748b" }}>
                  {t.comment || ""}
                </td>
              </tr>
            ))}
